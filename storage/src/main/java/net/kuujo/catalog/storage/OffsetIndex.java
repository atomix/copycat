...
import net.kuujo.catalyst.buffer.FileBuffer;
import net.kuujo.catalyst.buffer.MappedBuffer;
 ...
 * in a segment, the index will write the position to an underlying {@link net.kuujo.catalyst.buffer.Buffer}. With this information, the index provides
 ...
 * {@code 2^32 - 1}.
 * <p>
 * When the index is first created, the {@link net.kuujo.catalyst.buffer.Buffer} provided to the constructor will be scanned for existing entries.
 * <p>
 ...
 * is used to locate positions rather than absolute positions. For efficiency, a {@link net.kuujo.catalyst.buffer.MappedBuffer}
 * can be used to improve the performance of the binary search algorithm for persistent indexes.
 * <p>
 * In order to prevent searching the index for missing entries, all offsets are added to a memory efficient {@link net.kuujo.catalyst.buffer.util.BitArray}
 * as they're written to the index. The bit array is sized according to the underlying index buffer. Prior to searching
 * for an offset in the index, the {@link net.kuujo.catalyst.buffer.util.BitArray} is checked for existence of the offset in the index. Only if the offset
