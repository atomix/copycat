...
public abstract class AbstractStateTest<T extends AbstractState> extends ConcurrentTestCase {
  ...
  void beforeMethod() throws Throwable {
    ...
    serverState = new ServerState(members.get(0), members.stream().map(Member::serverAddress).collect(Collectors.toList()), members.size(), 1, log, stateMachine, new ConnectionManager(transport.client()), serverCtx);
  }
