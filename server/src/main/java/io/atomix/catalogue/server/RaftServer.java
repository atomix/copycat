...
import io.atomix.catalogue.server.state.ServerContext;
import io.atomix.catalogue.server.state.ServerState;
 ...
 * relation to {@link io.atomix.catalogue.client.Command}s and {@link io.atomix.catalogue.client.Query}s submitted to the
 * server by a client.
 * <pre>
 *   {@code
 ...
 *   }
 * </pre>
 * Server state machines are responsible for registering {@link io.atomix.catalogue.client.Command}s which can be submitted
 ...
 * {@link io.atomix.catalogue.server.RaftServer.Builder#withTransport(Transport)}.
 * <p>
 * As {@link io.atomix.catalogue.client.Command}s are received by the server, they're written to the Raft {@link io.atomix.catalogue.server.storage.Log}
 * and replicated to other members of the cluster. By default, the log is stored on disk, but users can override the default
 * {@link Storage} configuration via {@link io.atomix.catalogue.server.RaftServer.Builder#withStorage(Storage)}. Most notably,
 ...
 * can provide a custom serializer via {@link io.atomix.catalogue.server.RaftServer.Builder#withSerializer(Serializer)}.
...
public class RaftServer implements Managed<RaftServer> {
   ...
   * <p>
   * Throughout the lifetime of the cluster, the server will periodically transition between various {@link io.atomix.catalogue.server.RaftServer.State states}.
   * Users can listen for and react to state change events. To determine when this server is elected leader, simply
   * listen for the {@link io.atomix.catalogue.server.RaftServer.State#LEADER} state.
   * <pre>
   *   {@code
   *   server.onStateChange(state -> {
   *     if (state == RaftServer.State.LEADER) {
