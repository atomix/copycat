 ...
 * Raft server.
 * <p>
 * To create a new server, use the server {@link io.atomix.catalog.server.RaftServer.Builder}. Servers require
 ...
 * relation to {@link io.atomix.catalog.client.Command}s and {@link io.atomix.catalog.client.Query}s submitted to the
 * server by a client.
 * <pre>
 *   {@code
 ...
 *   }
 * </pre>
 * Server state machines are responsible for registering {@link io.atomix.catalog.client.Command}s which can be submitted
 ...
 * {@link io.atomix.catalog.server.RaftServer.Builder#withTransport(Transport)}.
 * <p>
 * As {@link io.atomix.catalog.client.Command}s are received by the server, they're written to the Raft {@link io.atomix.catalog.server.storage.Log}
 * and replicated to other members of the cluster. By default, the log is stored on disk, but users can override the default
 * {@link Storage} configuration via {@link io.atomix.catalog.server.RaftServer.Builder#withStorage(Storage)}. Most notably,
 * to configure the storage module to store entries in memory instead of disk, configure the
 * {@link io.atomix.catalog.server.storage.StorageLevel}.
 * <pre>
 *   {@code
 *   RaftServer server = RaftServer.builder(address, members)
 *     .withStateMachine(new MyStateMachine())
 ...
 * can provide a custom serializer via {@link io.atomix.catalog.server.RaftServer.Builder#withSerializer(Serializer)}.
...
public class RaftServer implements Managed<RaftServer> {
  ...
  public enum State {
     ...
     * <p>
     * All servers start in this state and return to this state when {@link RaftServer#close() stopped}.
     */
    INACTIVE,
     ...
     * <p>
     * When a server is {@link RaftServer#open() started}, the first state to which it transitions is the join state.
     ...
     * <p>
     * When a server is {@link RaftServer#close() stopped}, the server will first transition to the leave state and
     ...
     * exist at any given time, but Raft guarantees that no two leaders will exist for the same {@link RaftServer#term()}.
     */
    LEADER
   ...
   * <p>
   * Throughout the lifetime of the cluster, the server will periodically transition between various {@link io.atomix.catalog.server.RaftServer.State states}.
   * Users can listen for and react to state change events. To determine when this server is elected leader, simply
   * listen for the {@link io.atomix.catalog.server.RaftServer.State#LEADER} state.
   * <pre>
   *   {@code
   *   server.onStateChange(state -> {
   *     if (state == RaftServer.State.LEADER) {
