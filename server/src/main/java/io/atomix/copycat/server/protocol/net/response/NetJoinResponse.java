...
public class NetJoinResponse extends JoinResponse implements RaftNetResponse<NetJoinResponse> {
  ...
  public static class Serializer extends RaftNetResponse.Serializer<NetJoinResponse> {
    @Override
    public void writeObject(BufferOutput output, NetJoinResponse response) {
    ...
    public NetJoinResponse readObject(BufferInput input, Class<NetJoinResponse> type) {
      ...
      if (status == Status.OK) {
        ...
        return new NetJoinResponse(id, status, null, index, term, timestamp, members);
      } else {
