...
public class PollResponseSerializer extends RaftProtocolResponseSerializer<PollResponse> {
  @Override
  public void writeObject(BufferOutput output, PollResponse response) {
  ...
  @Override
  public PollResponse readObject(BufferInput input, Class<PollResponse> type) {
    final ProtocolResponse.Status status = ProtocolResponse.Status.forId(input.readByte());
    if (status == ProtocolResponse.Status.OK) {
      return new PollResponse(status, null, input.readLong(), input.readBoolean());
    } else {
      ...
      return new PollResponse(status, error, input.readLong(), input.readBoolean());
    }
  }
}