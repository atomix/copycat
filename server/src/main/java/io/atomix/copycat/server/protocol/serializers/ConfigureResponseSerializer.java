...
public class ConfigureResponseSerializer extends RaftProtocolResponseSerializer<ConfigureResponse> {
  @Override
  public void writeObject(BufferOutput output, ConfigureResponse response) {
  ...
  @Override
  public ConfigureResponse readObject(BufferInput input, Class<ConfigureResponse> type) {
    final ProtocolResponse.Status status = ProtocolResponse.Status.forId(input.readByte());
    if (status == ProtocolResponse.Status.OK) {
      return new ConfigureResponse(status, null);
    } else {
