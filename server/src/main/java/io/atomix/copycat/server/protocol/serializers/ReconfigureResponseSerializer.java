...
public class ReconfigureResponseSerializer extends RaftProtocolResponseSerializer<ReconfigureResponse> {
  @Override
  public void writeObject(BufferOutput output, ReconfigureResponse response) {
  ...
  public ReconfigureResponse readObject(BufferInput input, Class<ReconfigureResponse> type) {
    ...
    if (status == ProtocolResponse.Status.OK) {
      ...
      return new ReconfigureResponse(status, null, index, term, timestamp, members);
    } else {
      ...
      return new ReconfigureResponse(status, error, 0, 0, 0, null);
    }
  }
}