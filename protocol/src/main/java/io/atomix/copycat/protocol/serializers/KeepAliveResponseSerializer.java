...
public class KeepAliveResponseSerializer extends ProtocolResponseSerializer<KeepAliveResponse> {
  @Override
  public void writeObject(BufferOutput output, KeepAliveResponse response) {
  ...
  public KeepAliveResponse readObject(BufferInput input, Class<KeepAliveResponse> type) {
    ...
    if (status == ProtocolResponse.Status.OK) {
      ...
      return new KeepAliveResponse(status, null, leader, members);
    } else {
      ...
      return new KeepAliveResponse(status, error, null, null);
    }
  }
}